...
/** Dashboard-placeholder task shell + TaskDetail (same as Kanban). */
function placeholderTaskHrefCreated(row: TaskAPIResponse): string {
          ...
          <div className="relative z-20 flex shrink-0 flex-col gap-4 bg-white px-6 pt-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="rounded-[10px] bg-[#f0f3f5] p-[2px]">
                <button className={tabBtn(tab === "Deliverables")} onClick={() => setTab("Deliverables")}>
                  Deliverables
                </button>
          ...
          {tab !== "Deliverables" ? (
            <div className="scrollbar-none min-h-0 flex-1 overflow-y-auto overflow-x-clip px-6 pb-4 pt-4 overscroll-y-contain">
              <div className="flex flex-col items-center gap-4 py-16 text-[#727d83]">
                <div className="flex w-[286px] flex-col items-center gap-4">
                  <ListTodo size={48} />
                  <p className="text-[20px] font-bold">No Items Yet</p>
            ...
            <div className="scrollbar-none min-h-0 flex-1 overflow-y-auto overflow-x-clip px-6 pb-4 pt-4 overscroll-y-contain">
              <div className="flex flex-col items-center py-16 text-[#727d83]">
                <div className="flex w-[286px] flex-col items-center gap-4">
                  <ListTodo size={48} />
                  <p className="text-[20px] font-bold">No Tasks Created by You</p>
                  <p className="text-[14px]">Tasks you create will show up here.</p>
                </div>
              </div>
            </div>
          ) : (
                ...
                {tasks.map((row) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => navigate(placeholderTaskHrefCreated(row))}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          navigate(placeholderTaskHrefCreated(row));
                        }
                      }}
                    >
