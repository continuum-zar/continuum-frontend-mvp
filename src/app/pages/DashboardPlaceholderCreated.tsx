...
import { Ellipsis, Flag, GripVertical, ListTodo } from "lucide-react";
...
const DELIVERABLE_TASKS: CreatedTask[] = [
  {
    id: 1,
...
export function DashboardPlaceholderCreated() {
  const [tab, setTab] = useState<"Deliverables" | "Time logs" | "Activity">(
    "Deliverables"
  );

  const tasks = useMemo(() => (tab === "Deliverables" ? DELIVERABLE_TASKS : []), [tab]);
        ...
        <section className="z-[1] flex min-h-0 min-w-0 flex-1 flex-col gap-4 overflow-x-clip overflow-y-auto rounded-[8px] border border-[#ebedee] bg-white px-6 py-4 shadow-[0px_44px_12px_0px_rgba(15,15,31,0),0px_28px_11px_0px_rgba(15,15,31,0.01),0px_16px_10px_0px_rgba(15,15,31,0.02),0px_7px_7px_0px_rgba(15,15,31,0.03),0px_2px_4px_0px_rgba(15,15,31,0.04)]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <p className="text-[32px] font-medium text-[#0b191f]">Created by Me</p>
              <Ellipsis size={16} className="text-[#0b191f]" />
            </div>
            <div className="rounded-[10px] bg-[#f0f3f5] p-[2px]">
              <button
                className={tabBtn(tab === "Deliverables")}
          ...
          {tasks.length === 0 ? (
            <div className="flex flex-1 items-center justify-center">
              <div className="flex flex-col items-center gap-4 text-[#727d83]">
                <ListTodo size={48} />
                <p className="text-[20px] font-bold">No Home Items</p>
                <p className="text-[14px]">Add home items</p>
              </div>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-center">
                    {row.assignees.map((a, i) => (
                      <div
                        key={a}
                        className={`-mr-2 flex h-6 w-6 items-center justify-center rounded-full border border-white text-[9px] text-white ${
                          i === 0 ? "bg-[#e19c02]" : i === 1 ? "bg-[#f17173]" : "bg-[#9da2f7]"
                        }`}
                      >
                        {a}
                      </div>
