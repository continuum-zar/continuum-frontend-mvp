...
export function ClerkAccountSettings() {
  ...
  if (!isLoaded) {
    return (
      <div className="font-['Satoshi',sans-serif] text-[14px] text-[#606d76]">
        Loading account…
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 font-['Satoshi',sans-serif] text-[14px] text-[#0b191f]">
      <section className="flex flex-col gap-2">
        <h3 className="text-[16px] font-medium">Signed in as</h3>
        <p className="text-[14px] text-[#606d76]">{primaryEmail}</p>
      </section>
        ...
        {externalProviders.length === 0 && !hasPasswordIdentity ? (
          ...
          <ul className="flex flex-col gap-2">
            {externalProviders.map((entry) => (
              <li
                key={entry.id}
                className="flex items-center justify-between rounded-[8px] border border-[#ebedee] bg-white px-3 py-2"
              >
                <div className="flex flex-col">
                  <span className="text-[14px] font-medium">{entry.label}</span>
                  {entry.email ? (
                    <span className="text-[12px] text-[#606d76]">{entry.email}</span>
                  ) : null}
                </div>
                <span className="text-[12px] uppercase tracking-wide text-[#606d76]">Linked</span>
              </li>
            ))}
            {hasPasswordIdentity ? (
              <li className="flex items-center justify-between rounded-[8px] border border-[#ebedee] bg-white px-3 py-2">
                <div className="flex flex-col">
                  <span className="text-[14px] font-medium">Email &amp; password</span>
                  <span className="text-[12px] text-[#606d76]">{primaryEmail}</span>
                </div>
                <span className="text-[12px] uppercase tracking-wide text-[#606d76]">Enabled</span>
              </li>
            ) : null}
          </ul>
        )}
        <p className="text-[12px] text-[#606d76]">
