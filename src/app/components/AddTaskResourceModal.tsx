...
export function AddTaskResourceModal({
  ...
  const dropHint =
    pendingFiles.length === 0 ? (
      ...
      <span className="text-muted-foreground">Add more files below, or drop to append</span>
    );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          ...
          className={cn(
            "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-1/2 left-1/2 z-50 flex max-h-[min(90vh,720px)] w-[calc(100%-2rem)] max-w-[560px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-[16px] border border-border bg-card shadow-[0px_39px_11px_0px_rgba(181,181,181,0),0px_25px_10px_0px_rgba(181,181,181,0.04),0px_14px_8px_0px_rgba(181,181,181,0.12),0px_6px_6px_0px_rgba(181,181,181,0.2),0px_2px_3px_0px_rgba(181,181,181,0.24)] duration-200",
          )}
        >
          <DialogPrimitive.Title className="sr-only">Resources</DialogPrimitive.Title>

          <div className="z-[3] grid w-full shrink-0 grid-cols-[1fr_auto_1fr] items-center border-b border-solid border-border bg-card px-9 py-4">
            ...
            <div className="flex w-full flex-col gap-6 pb-6">
              <div className="flex w-full flex-col gap-2">
                ...
                >
                  ...
                  <p className="max-w-full text-center font-['Satoshi',sans-serif] text-[13px] font-medium text-muted-foreground">
                    {dropHint}
                  </p>
                </div>
                <div
                  className="flex w-full items-start justify-between font-['Satoshi',sans-serif] text-[14px] font-normal whitespace-nowrap text-muted-foreground"
                ...
                {pendingFiles.length > 0 ? (
                  <ul className="flex w-full flex-col gap-2 rounded-[10px] border border-border bg-card p-2" aria-label="Selected files">
                    {pendingFiles.map(({ clientId, file }) => (
                      <li
                        key={clientId}
                        className="flex items-start gap-2 rounded-[8px] bg-card px-3 py-2 font-['Satoshi',sans-serif] text-[13px] text-foreground"
                      >
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-medium">{file.name}</p>
                          <p className="text-[12px] text-muted-foreground">{formatShortFileSize(file.size)}</p>
                          {perFileErrors[clientId] ? (
                            <p className="mt-1 text-[12px] font-medium text-destructive">{perFileErrors[clientId]}</p>
                          ) : null}
                        </div>
                        <button
                          type="button"
                          className="inline-flex shrink-0 rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-40"
              ...
              <div className="flex w-full items-center gap-4">
                <div className="h-px min-h-px flex-1 bg-muted" />
                <p className="shrink-0 font-['Satoshi',sans-serif] text-[16px] font-medium text-muted-foreground">Or</p>
                <div className="h-px min-h-px flex-1 bg-muted" />
              </div>

              <div className="flex w-full flex-col gap-1">
                <p className="font-['Satoshi',sans-serif] text-[14px] font-medium text-muted-foreground">Paste a link</p>
                <div className="flex h-10 w-full items-center rounded-[8px] border border-solid border-border bg-card px-4 py-2">
                  <input
                    type="url"
                    ...
                    className="w-full min-w-0 border-0 bg-transparent font-['Satoshi',sans-serif] text-[16px] font-medium text-foreground outline-none placeholder:text-muted-foreground disabled:opacity-50"
                  />
                </div>
              </div>
