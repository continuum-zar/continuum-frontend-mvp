...
function MermaidDiagram({ code }: { code: string }) {
    ...
    if (error) {
        return (
            <div className="my-3 rounded-lg border border-dashed border-[#fecaca] bg-[#fef2f2] p-3 font-mono text-[12px] text-[#b91c1c]">
                Mermaid render failed: {error}
                <pre className="mt-2 overflow-x-auto whitespace-pre-wrap font-mono text-[11px] text-[#7f1d1d]">
                    {code}
                </pre>
            </div>
        );
    ...
    if (!svg) {
        return (
            <div className="my-3 flex items-center gap-2 rounded-lg border border-[#edecea] bg-[#f9fafb] p-3 font-['Satoshi',sans-serif] text-[13px] text-[#606d76]">
    ...
    return (
        <div
            className="my-3 overflow-x-auto rounded-lg border border-[#edecea] bg-white p-3"
...
const baseText =
    "font-['Inter',sans-serif] text-[14px] font-normal leading-[22px] text-[#0b191f]";

const markdownComponents: Components = {
    h1: ({ children }) => (
        <h1 className="mb-3 mt-5 first:mt-0 font-['Satoshi',sans-serif] text-[22px] font-semibold text-[#0b191f]">
            {children}
        </h1>
    ),
    h2: ({ children }) => (
        <h2 className="mb-2 mt-5 first:mt-0 font-['Satoshi',sans-serif] text-[18px] font-semibold text-[#0b191f]">
            {children}
        </h2>
    ),
    h3: ({ children }) => (
        <h3 className="mb-2 mt-4 first:mt-0 font-['Satoshi',sans-serif] text-[16px] font-semibold text-[#0b191f]">
            {children}
        </h3>
    ),
    h4: ({ children }) => (
        <h4 className="mb-1.5 mt-3 first:mt-0 font-['Satoshi',sans-serif] text-[14px] font-semibold text-[#0b191f]">
            {children}
        </h4>
    ),
    ...
    ul: ({ children }) => (
        <ul className="mb-2 ml-5 list-disc space-y-1 marker:text-[#606d76] last:mb-0">
            {children}
        </ul>
    ),
    ol: ({ children }) => (
        <ol className="mb-2 ml-5 list-decimal space-y-1 marker:font-medium marker:text-[#606d76] last:mb-0">
            {children}
        </ol>
    ),
    ...
    thead: ({ children }) => (
        <thead className="bg-[#f4f5f7] text-[#0b191f]">{children}</thead>
    ),
    th: ({ children }) => (
        <th className="border-b border-[#edecea] px-3 py-2 font-semibold">{children}</th>
    ),
    td: ({ children }) => (
        <td className="border-b border-[#edecea] px-3 py-2 align-top">{children}</td>
    ),
    ...
    pre: ({ children }) => (
        <pre className="mb-3 overflow-x-auto rounded-lg border border-[#edecea] bg-[#f9fafb] p-3 font-mono text-[12px] leading-[18px] text-[#0b191f] last:mb-0">
            {children}
        </pre>
    ),
};
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent
                ...
                hideClose
            >
                <DialogHeader className="flex flex-row items-center justify-between gap-3 border-b border-[#edecea] p-4">
                    <DialogTitle className="font-['Satoshi',sans-serif] text-[18px] font-semibold text-[#0b191f]">
                        {title}
                    </DialogTitle>
                    <div className="flex items-center gap-1.5">
                        ...
                        <DialogClose asChild>
                            <button
                                type="button"
                                aria-label="Close"
                                className="inline-flex size-8 items-center justify-center rounded-md text-[#606d76] hover:bg-[#edf0f3] hover:text-[#0b191f]"
                            >
                ...
                <div className="min-h-[200px] flex-1 overflow-y-auto px-6 py-5">
                    {loading ? (
                        <div className="flex items-center gap-2 font-['Satoshi',sans-serif] text-[14px] text-[#606d76]">
                            <Loader2 className="size-4 animate-spin" strokeWidth={2} />
                            Loading resource…
                        </div>
                    ) : error ? (
                        <div className="rounded-lg border border-dashed border-[#fecaca] bg-[#fef2f2] p-4 font-['Satoshi',sans-serif] text-[14px] text-[#b91c1c]">
                            {error}
                        </div>
                    ) : markdown == null ? null : (
